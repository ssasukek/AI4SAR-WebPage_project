...
const IncidentCard: React.FC<IncidentCardProps> = ({
  ...
  return (
    <div
      ...
      onClick={() => navigate.push(`/private/incidents/${incident.id}`)}
    >
      <Card>
...
const IncidentsList: React.FC = () => {
  ...
  const sortedNamesDESC = [...incidents].sort((a, b) =>
    b.data.incidentName
      .toLowerCase()
      .localeCompare(a.data.incidentName.toLowerCase()),
  );

  const sortedDateLatest = [...incidents].sort((a, b) => {
    let dateA = a.data.incidentDate
      ? new Date(a.data.incidentDate)
      : new Date(0);
    let dateB = b.data.incidentDate
      ? new Date(b.data.incidentDate)
      : new Date(0);
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      or(
        where("viewableBy", "array-contains", user.id),
        where("uid", "==", userId),
      ...
      for (const otherFolder of folders) {
        if (otherFolder.incidentIds.includes(incidentId)) {
          await updateDoc(doc(db, "folders", otherFolder.id), {
            incidentIds: otherFolder.incidentIds.filter(
              (id) => id !== incidentId,
            ),
          });
        }
  ...
  return (
    <>
      ...
      >
        <Button
          onClick={() => navigate.push(`/private/incidents/creation`)}
        ...
        {display
          .filter(
            (incident) =>
              ...
              !folders.some((folder) =>
                folder.incidentIds.includes(incident.id),
              ),
          )
          .map((incident, i) => (
            <div
      ...
      <Modal
        ...
        toggle={() => setShowNewFolderModal(false)}
      >
